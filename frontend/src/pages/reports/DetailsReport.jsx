...
export default function DetailsReport() {
  ...
  const getReportParams = () => {
    ...
    params.type = 'visitors';
    if (selectedChannels.length > 0) {
      params.channel = selectedChannels.join(',');
    }
    return params;
  };

  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ['details-report', selectedDkey, startDate, endDate, selectedChannels],
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-6">
        <h1 className="text-xl font-bold text-secondary-900 mb-6">Details Report Input</h1>
                ...
                {showChannelDropdown && filteredChannels.length > 0 && (
                  <div className="absolute z-50 mt-1 w-full max-w-md bg-white border border-secondary-200 rounded-lg shadow-lg max-h-48 overflow-auto">
                    {filteredChannels.slice(0, 20).map((c, idx) => (
                      <button
                        key={idx}
                        onClick={() => handleAddChannel(c.channel)}
              ...
              {selectedChannels.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {selectedChannels.map(channel => (
                    <span
                      key={channel}
                      className="inline-flex items-center gap-1 px-3 py-1 bg-primary-50 border border-primary-200 rounded text-sm text-primary-700"
                    >
                      {channel}
                      <button
